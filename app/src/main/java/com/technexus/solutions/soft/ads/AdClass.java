...
public class AdClass {
    ...
    public static void showInterAd(Activity activity, OnInterClose onInterClose) {
        if (SharePrefs.getInstance(activity).getBoolean(Utils.ADS_PRELOAD)) {
            if (activity != null && SharePrefs.getInstance(activity).getBoolean(Utils.ADS_SHOW) && SharePrefs.getInstance(activity).getBoolean(Utils.INTER_ADS_SHOW) && SharePrefs.getInstance(activity).getInt(Utils.ADS_COUNT) == ads_count) {
                ads_count = 0;
                if (SharePrefs.getInstance(activity).getBoolean(Utils.ONLY_URL)) {
                ...
                } else if (SharePrefs.getInstance(activity).getBoolean(Utils.REMOVE_FACEBOOK)) {
                    if (mInterstitialAd != null) {
                        mInterstitialAd.setFullScreenContentCallback(new FullScreenContentCallback() {
                            ...
                            public void onAdDismissedFullScreenContent() {
                                ...
                                onInterClose.onInterClose();
                                AdClass.preLoadInterAds(activity);
                            }
    ...
    public static void preLoadInterAds(Activity activity) {
        if (SharePrefs.getInstance(activity).getBoolean(Utils.ADS_PRELOAD)) {
            if (activity != null && SharePrefs.getInstance(activity).getBoolean(Utils.ADS_SHOW) && SharePrefs.getInstance(activity).getBoolean(Utils.INTER_ADS_SHOW)) {
                if (SharePrefs.getInstance(activity).getBoolean(Utils.REMOVE_FACEBOOK)) {
    ...
    private static void preLoadGoogle(Activity activity) {
        InterstitialAd.load(activity, SharePrefs.getInstance(activity).getString(Utils.INTERSTITIAL_AD_ID), adRequest,
    ...
    private static void preLoadFacebook(Activity activity) {
        fbInter = new com.facebook.ads.InterstitialAd(activity, SharePrefs.getInstance(activity).getString(Utils.FB_INTER_AD_ID));
    ...
    public static void loadAndShowInterAd(Activity activity, OnInterClose onInterClose) {
        if (activity != null && SharePrefs.getInstance(activity).getBoolean(Utils.ADS_SHOW) && SharePrefs.getInstance(activity).getBoolean(Utils.INTER_ADS_SHOW) && SharePrefs.getInstance(activity).getInt(Utils.ADS_COUNT) == ads_count) {
            ...
            if (SharePrefs.getInstance(activity).getBoolean(Utils.REMOVE_FACEBOOK)) {
                loadAndShowGoogleInter(activity, onInterClose, updateDialog, SharePrefs.getInstance(activity).getString(Utils.INTERSTITIAL_AD_ID));
            } else {
...
//                                    loadInter(activity);
    ...
    public static void showBanner(Activity context, LinearLayout layout) {
        ...
        adView.setAdUnitId(SharePrefs.getInstance(context).getString(Utils.BANNER_AD_ID));
    ...
    public static void loadBannerAd(Activity activity, LinearLayout linearLayout, LinearLayout linearQureka) {
        if (activity != null && SharePrefs.getInstance(activity).getBoolean(Utils.ADS_SHOW) && SharePrefs.getInstance(activity).getBoolean(Utils.BANNER_ADS_SHOW)) {
            if (SharePrefs.getInstance(activity).getBoolean(Utils.ONLY_URL)) {
    ...
    private static void showFacebookBanner(Activity activity, LinearLayout linearLayout) {
        bannerAdViewFB = new com.facebook.ads.AdView(activity, SharePrefs.getInstance(activity).getString(Utils.FB_BANNER_AD_ID), com.facebook.ads.AdSize.BANNER_HEIGHT_50);
    ...
    private static void loadGoogleNative(Activity activity, FrameLayout flNative, int view, ShimmerFrameLayout layout, String adId) {
        ...
        AdLoader.Builder builder = new AdLoader.Builder(activity, SharePrefs.getInstance(activity).getString(Utils.NATIVE_AD_ID));
    ...
    public static void showNativeAd(Activity context, FrameLayout flNative, int view, NativeAdLayout fbNativeAdLayout, int fbView, ShimmerFrameLayout layout, int adSize, RelativeLayout relativeNative) {
        if (context != null && SharePrefs.getInstance(context).getBoolean(Utils.ADS_SHOW) && SharePrefs.getInstance(context).getBoolean(Utils.NATIVE_ADS_SHOW)) {
            if (SharePrefs.getInstance(context).getBoolean(Utils.ONLY_URL)) {
                ...
                relativeNative.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        CustomTabsIntent customTabsIntent = builder.build();
                        try {
                            customTabsIntent.launchUrl(context, Uri.parse(SharePrefs.getInstance(context).getString(Utils.URL_LINK)));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                });
            } else if (SharePrefs.getInstance(context).getBoolean(Utils.REMOVE_FACEBOOK)) {
                loadGoogleNative(context, flNative, view, layout, SharePrefs.getInstance(context).getString(Utils.NATIVE_AD_ID));
            } else {
    ...
    public static void loadFacebookNative(Activity activity, FrameLayout flNative, int view, NativeAdLayout fbNativeAdLayout, int fbView, ShimmerFrameLayout layout, int adSize) {
        ...
        com.facebook.ads.NativeAd nativeAd = new com.facebook.ads.NativeAd(activity, SharePrefs.getInstance(activity).getString(Utils.FB_NATIVE_AD_ID));
        ...
        NativeAdListener nativeAdListener = new NativeAdListener() {
            ...
            public void onError(Ad ad, com.facebook.ads.AdError adError) {
                if (!adLoadComplete) {
                    loadGoogleNative(activity, flNative, view, layout, SharePrefs.getInstance(activity).getString(Utils.NATIVE_AD_ID));
                } else {
                    if (layout != null) {
                        layout.setVisibility(View.GONE);
                    }
...
//        nativeAdSocialContext.setText(nativeAd.getAdSocialContext());
    ...
    public static void loadGoogleRectangleBanner(Activity activity, ShimmerFrameLayout layout, LinearLayout linearLayout, ProgressBar progressBar) {
        if (activity != null && SharePrefs.getInstance(activity).getBoolean(Utils.ADS_SHOW)) {
            AdView mAdView = new AdView(activity);
            mAdView.setAdSize(AdSize.MEDIUM_RECTANGLE);
            mAdView.setAdUnitId(SharePrefs.getInstance(activity).getString(Utils.BANNER_AD_ID));
